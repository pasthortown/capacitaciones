...
/** Formatea un entero como `CAP-PC-REG-###` (3 dígitos con ceros). */
function formatCodigo(numero, formato = 'CAP-PC-REG-###') {
  if (numero === null || numero === undefined || Number.isNaN(Number(numero))) return formato;
...
/** Tab de numeración de capacitaciones (CAP-PC-REG-###). */
export default function NumeracionCapacitacionesTab() {
  ...
  const parseInput = () => {
    ...
    if (!trimmed) return { error: 'Ingresa un número entre 1 y 999.' };
    const parsed = Number(trimmed);
    if (!Number.isInteger(parsed)) return { error: 'El valor debe ser un entero.' };
    if (parsed < 1 || parsed > 999) return { error: 'El valor debe estar entre 1 y 999.' };
    return { value: parsed };
  };
      ...
      <div className="card">
        <div className="card__header">
          ...
          <p className="card__subtitle">
            Define el siguiente número que se asignará. Bajar el valor puede generar
            colisiones con capacitaciones ya emitidas.
          </p>
        </div>
        <div className="card__body">
        ...
        <div className="alert alert--warning" role="alert" style={{ marginBottom: 'var(--spacing-3)' }}>
          ...
          <div className="alert__content">
            ...
            <div className="alert__message">
              Bajar el contador puede causar colisiones con capacitaciones existentes. ¿Deseas continuar?
            </div>
          </div>
        </div>
        <p>
