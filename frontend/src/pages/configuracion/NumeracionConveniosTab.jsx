...
/** Formatea un entero como `GIC-EC-REG-###` (mínimo 3 dígitos, sin tope superior). */
function formatCodigo(numero) {
  if (numero === null || numero === undefined || Number.isNaN(Number(numero))) return 'GIC-EC-REG-###';
  return `GIC-EC-REG-${String(Math.max(0, Number(numero))).padStart(3, '0')}`;
}
...
/** Tab de numeración de convenios (GIC-EC-REG-###). */
export default function NumeracionConveniosTab() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const response = await conveniosService.obtenerNumeracion();
      setData(response);
  ...
  const parseInput = () => {
    ...
    if (!trimmed) return { error: 'Ingresa un número ≥ 1.' };
    const parsed = Number(trimmed);
    if (!Number.isInteger(parsed)) return { error: 'El valor debe ser un entero.' };
    if (parsed < 1) return { error: 'El valor debe ser ≥ 1.' };
    return { value: parsed };
  };
  ...
  const doSave = async (value) => {
    setSaving(true);
    try {
      await conveniosService.actualizarNumeracion(value);
      toast.success('Numeración actualizada.');
      await fetchData();
    } catch (error) {
      if (error instanceof HttpError && error.status === 400) {
        const backendMsg = error?.body?.message || error?.body?.title || error?.message;
  ...
  const confirmDowngrade = async () => {
    ...
    if (value !== null) await doSave(value);
  };
  ...
  const formato = data?.formato || 'GIC-EC-REG-###';
  const proximoCodigo = data?.siguienteCodigo || formatCodigo(data?.siguienteNumero);
  ...
  return (
    <div>
        ...
        <div className="card__body">
          <div style={{ display: 'grid', gap: 'var(--spacing-3)' }}>
            <div>
              ...
              <div style={{ fontFamily: 'var(--font-family-display)', fontSize: 'var(--font-size-lg)' }}>{formato}</div>
            </div>
            <div>
              <div className="text-xs text-secondary">Próximo código a asignar</div>
              <div style={{
                fontFamily: 'var(--font-family-display)', fontSize: 'var(--font-size-2xl)',
      ...
      <div className="card">
        <div className="card__header">
          ...
          <p className="card__subtitle">
            Define el siguiente número que se asignará. Debe ser mayor al último código
            de convenio ya emitido.
          </p>
        </div>
        <div className="card__body">
          <form onSubmit={handleSubmit} noValidate>
            <div style={{ maxWidth: 280 }}>
              <TextField
                label="Siguiente número (≥ 1)"
                name="siguienteNumeroConvenio"
                type="number"
                value={inputValue}
      ...
      <Modal
        ...
        footer={(
          <>
            <button type="button" className="btn btn--ghost" onClick={cancelDowngrade} disabled={saving}>Cancelar</button>
        ...
        <div className="alert alert--warning" role="alert" style={{ marginBottom: 'var(--spacing-3)' }}>
          ...
          <div className="alert__content">
            ...
            <div className="alert__message">
              Bajar el contador puede causar colisiones con convenios existentes. ¿Deseas continuar?
            </div>
          </div>
        </div>
        <p>
